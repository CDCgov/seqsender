			...
			case $config_path in 
				...
				*) 
					...
					then
						...
						python $RESOURCE_ROOT/extract_config.py --config $config_file
						...
						arg_list+="--config default_config.yaml "
						i=$(($i+2))
					else
...
case $provided_metadata in
	...
	*)
		...
		python $RESOURCE_ROOT/extract_metadata.py --metadata $provided_metadata --config $config_file
...
case $provided_config in 
	...
	*)
		...
		then
			...
			python $RESOURCE_ROOT/update_upload_log.py --log $RESOURCE_ROOT/upload_log.csv --config $provided_config --unique_name $unique_name
...
# Copy test_input directory back to working directory so users can access it
