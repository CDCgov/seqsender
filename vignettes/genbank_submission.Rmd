...
Before one can batch uploading meta- and sequence-data to **`r title`** database using ``r program``, they must ensure the requirement files (such as `config.yaml`, `metadata.csv`, `sequence.fasta`, `raw reads`, etc.) are already prepared and stored in a submission directory of choice.
