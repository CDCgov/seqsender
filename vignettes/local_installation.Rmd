...
After you've finished prepping for your databases, create a submission folder and store your metadata and sequence files there.
...
### <a onclick="ToggleOperation('manual')"><i class="fas fa-play" role="presentation" aria-label="play icon"></i> [Optional] Make a manual submission.</a>

<div id="manual-block" style="display: none;">

If you'd not like to use the automated upload feature of ``r program`` you can still use it to create the files for manual submission to each database.
...
#### 2. Prepare your files for submission.

```bash
python seqsender.py prep \
--organism FLU \
-bsng \
--submission_dir $HOME \
--submission_name flu-test-submission \
...
#### 3. Upload your files to each database.

After prepping all of your files. You're ready to manually upload your files to each database.

**BioSample**
Note: If you're also uploading to SRA then skip this step and attach your biosample `metadata.tsv` to your SRA submission.

For your BioSample submission you'll only need the `metadata.tsv`.
Here is an example of where the file is and where to attach for your manual submission:

**SRA**
Note: If you're also uploading to BIoSample then attach your biosample `metadata.tsv` to your SRA submission.

Here is an example of where the file is and where to attach for your manual submission:

**GenBank**

Here is an example of where the files are and where to attach for your manual submission:
 attach to your manual submission:
 
**GISAID**

For your GISAID submission you'll need the `metadata.csv` and `sequence.fsa` files.
Here is an example of where the files are and where to attach for your manual submission:
