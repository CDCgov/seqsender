...
For all GISAID submissions, ``r program`` makes use of GISAID's Command Line Interface Tools (e.g. ``r paste0(cli_list, collapse=", ")``, etc.) to batch uploading meta- and sequence-data to their databases. Prior to perform a batch upload, submitter must download the ``r paste(title, "CLI")`` package from the GISAID platform that is compatible with their machine and stored it in a subfolder called `gisaid_cli` within a submission directory of choice.

![](images/`r cli`_download.png)
<br>

After unzipped and extracted the CLI from the downloaded package, next, submitter will need to prepare the requirement files to pass to ``r program``.
