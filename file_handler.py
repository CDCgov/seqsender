...
# Prevent overwriting a file if it could cause the user to have a submission error
def allow_file_overwrite(file_path: str, allow_overwrite: bool):
	if os.path.isfile(file_path) and allow_overwrite == False:
		logger.error(f"There appears to already be a submission present at: {file_path}")
		logger.error("If you want to overwrite this submission directory, rerun the script again with the flag `--skip_overwrite_check`.")
		logger.error("If you want to update this current submission to include an additional database, rerun the script again with the flag `--update_submission`.")
		logger.error("Otherwise, use a different `--submission_name` to not overwrite this submission..")
		sys.exit(1)

def copy_file(source: str, destination: str):
	logger.debug(f"Copying file: {source}")
	shutil.copy(source, destination)
	logger.debug(f"Filed copied to: {destination}")
...
def validate_gisaid_installer(submission_dir: str, organism: str) -> str:
	...
	if os.path.isfile(gisaid_cli_path_option_one):
		logger.debug(f"GISAID CLI path selected: {gisaid_cli_path_option_one}")
		return gisaid_cli_path_option_one
	elif os.path.isfile(gisaid_cli_path_option_two):
		logger.debug(f"GISAID CLI path selected: {gisaid_cli_path_option_two}")
		return gisaid_cli_path_option_two
	elif os.path.isfile(gisaid_cli_path_option_three):
		logger.debug(f"GISAID CLI path selected: {gisaid_cli_path_option_three}")
		return gisaid_cli_path_option_three
	elif os.path.isfile(gisaid_cli_path_option_four):
		logger.debug(f"GISAID CLI path selected: {gisaid_cli_path_option_four}")
		return gisaid_cli_path_option_four
	else:
		logger.error(f"There is not a GISAID CLI for {organism} located at: '{gisaid_cli_path_option_one}' or '{gisaid_cli_path_option_two}'")
		logger.error(f"Download the GISAID CLI for {organism} from \"https://gisaid.org/\".")
		logger.error(f"Extract the zip file and place the CLI binary at either: '{gisaid_cli_path_option_one}' or '{gisaid_cli_path_option_two}'")
		sys.exit(1)
...
def load_yaml(yaml_type: str, yaml_path: str):
	...
	with open(yaml_path, "r") as file:
		try:
			...
			logger.error(f"'{yaml_type}' is incorrect. File must be a valid yaml format.", file=sys.stderr)
			sys.exit(1)
	logger.debug(f"YAML file successfully loaded.")
	return config_dict

def save_text(text_content: List[str], submission_dir: str, file_name: str) -> None:
	logger.debug(f"Saving text: {os.path.join(submission_dir, file_name)}")
	with open(os.path.join(submission_dir, file_name), "w+") as file:
		for line in text_content:
			file.write(line + "\n")
	logger.debug("Text file saved.")
...
def load_csv(file_path: str, sep: str = ",") -> pd.DataFrame:
	...
	logger.debug(f"CSV file successfully loaded into dataframe.")
	return df

# Load fasta file into pandas df and clean then return
def load_fasta_file(fasta_file: str) -> pd.DataFrame:
	fasta_dict = []
	logger.debug(f"Loading fasta file: {fasta_file}")
	...
	logger.debug("Fasta file successfully loaded into dataframe.")
	return fasta_df

# Save submission xml
def save_xml(submission_xml: bytes, submission_dir: str, file_name: str = "submission.xml") -> None:
	# Save string as submission.xml
	try:
		logger.debug(f"Saving XML: {os.path.join(submission_dir, file_name)}")
		with open(os.path.join(submission_dir, file_name), "wb") as file:
			file.write(submission_xml)
	except PermissionError as e:
		logger.error(f"Permission error when trying to save '{file_name}' to path: {submission_dir} \n{e}")
		sys.exit(1)
	except Exception as e:
		logger.critical(f"An unexpected error occurred when trying to save '{file_name}' to path: {submission_dir} \n{e}")
		sys.exit(1)
...
def save_csv(df: pd.DataFrame, file_path: str, file_name: Optional[str] = None, sep: str = ",") -> None:
	...
	try:
		logger.debug(f"Saving CSV: {file_path}")
		df.to_csv(file_path, header = True, index = False, sep = sep)
	except PermissionError as e:
		logger.error(f"Permission error when trying to save '{file_name}' to path: {file_path}\n{e}")
		sys.exit(1)
	except Exception as e:
		logger.critical(f"Error: An unexpected error occurred when trying to save '{file_name}' to path: {file_path} \n{e}")
		sys.exit(1)
...
def create_fasta(database: str, metadata: pd.DataFrame, submission_dir: str) -> None:
	logger.debug(f"Saving FASTA: {os.path.join(submission_dir, 'sequence.fsa')}")
	records = []
	...
	try:
		...
		logger.error(f"Permission error when trying to save 'sequence.fsa' to path: {submission_dir} \n{e}")
		sys.exit(1)
	except Exception as e:
		logger.critical(f"An unexpected error occurred when trying to save 'sequence.fsa' to path: {submission_dir} \n{e}")
		sys.exit(1)
	logger.debug("FASTA saved.")
