...
def main():
    ...
    if os.path.isfile(os.path.join(os.path.dirname(os.path.abspath(__file__)), "upload_log.csv")):
        ...
        print("\n" + "Error: Either a submission has not been made or upload_log.csv has been moved from current working directory" + "\n")
        sys.exit(1)

    if df["name"].str.contains(unique_name).any():
        df = df[df["name"].str.contains(unique_name)]
