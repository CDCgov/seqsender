    ...
    if len(df.index) != 0:
        for index, row in df.iterrows():
            ...
            if report_generated == True:
                ...
                if submission_status == "processed-ok" and config_dict["general"]["submit_Genbank"].lower() == "true" and (row["Genbank_status"] == None or row["Genbank_status"] == "" or pd.isnull(row["Genbank_status"])):
...
def submit_gisaid(unique_name, config, test):
    initialize_global_variables(config)
    if config_dict["gisaid"]["Update_sequences_on_Genbank_auto_removal"].lower() == "true" and config_dict["ncbi"]["Genbank_auto_remove_sequences_that_fail_qc"].lower() == "true":
...
def start_submission(unique_name, config, test, overwrite):
    ...
    elif config_dict["general"]["submit_BioSample"].lower() == "true" and config_dict["general"]["submit_SRA"].lower() == "true" and config_dict["general"]["joint_SRA_BioSample_submission"].lower() == "false":
    ...
    elif config_dict["general"]["submit_BioSample"].lower() == "true":
        submit_biosample_sra(unique_name, config, test, "biosample", overwrite)
    elif config_dict["general"]["submit_SRA"].lower() == "true":
        submit_biosample_sra(unique_name, config, test, "sra", overwrite)
    elif config_dict["general"]["submit_Genbank"].lower() == "true":
        submit_genbank(unique_name=unique_name, config=config, test=test, overwrite=overwrite)
    elif config_dict["general"]["submit_GISAID"].lower() == "true":
